...
object Keystore {
  ...
  lazy val Directory : Failable[File] = {
    repository.Directory.flatMap( mainDir => ensureUserOnlyDirectory( new File( mainDir, Keystore.DirName ) ) )
  }
  ...
  final object V3 {
    ...
    lazy val Directory : Failable[File] = {
      Keystore.Directory.flatMap( mainDir => ensureUserOnlyDirectory( new File( mainDir, V3.DirName ) ) )
    }
