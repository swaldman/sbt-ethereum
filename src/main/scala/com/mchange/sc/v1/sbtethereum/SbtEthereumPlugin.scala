...
object SbtEthereumPlugin extends AutoPlugin {
  ...
  private val Mutables = new mutables.Mutables (
    ...
    keystoresV3Finder    = activeShoeboxKeystoreV3,
   ...
   */ 
  def activeShoeboxKeystoreV3() = activeShoebox.keystore.V3.Directory.map( dir => immutable.Seq( dir ) ).assert
  ...
  object autoImport {
    ...
    val ethShoeboxDirectoryPrint  = taskKey[Unit] ("Prints the loction of the sbt-ethereum shoebox currently in use.")
    val ethShoeboxDirectorySwitch = inputKey[Unit]("Resets the current session to use a given directory as the shoebox directory.")
    val ethShoeboxSqlDumpCreate   = taskKey[Unit] ("Dumps the sbt-ethereum shoebox database as an SQL text file, stored inside the sbt-ethereum shoebox directory.")
    val ethShoeboxSqlDumpRestore  = taskKey[Unit] ("Restores the sbt-ethereum shoebox database from a previously generated dump.")
  ...
  lazy val ethDefaults : Seq[sbt.Def.Setting[_]] = Seq(
    ...
    ethcfgKeystoreAutoImportLocationsV3 := Nil, // used to default to importing from default geth keystore directory, but that seems surprising and heavyhanded now
    ...
    ethShoeboxDirectoryPrint := { ethShoeboxDirectoryPrintTask.value },

    ethShoeboxDirectorySwitch := { ethShoeboxDirectorySwitchTask.evaluated },

    ethShoeboxRestore := { ethShoeboxRestoreTask.value },

    ethShoeboxSqlDumpCreate := { ethShoeboxSqlDumpCreateTask.value },

    ethShoeboxSqlDumpRestore := { ethShoeboxSqlDumpRestoreTask.value },
    ...
    Def.inputTask {
      ...
      def doFindRent( name : String, rmd : ensClient.RegistrarManagedDomain ) : Unit = {
        ...
        val DurationParsers.SecondsViaUnit(seconds, unit) = {
          queryDurationInSeconds( is, """For how long would you like to rent the name (ex: "3 years")? """ ).getOrElse( aborted( "User failed to supply a desired time interval." ) )
        }
      ...
      def doNameCommitRegister( name : String, rmd : ensClient.RegistrarManagedDomain ) : Unit = {
        ...
        def doQueryDurationInSecondsPaymentInWei : ( Long, BigInt ) = {
          ...
          if ( seconds < minTime ) {
            log.warn( s"Registration period must be longer than ${formatDurationInSeconds(minTime.toLong, unit)}." )
            log.warn( s"You cannot register for just ${desiredPeriod}, although you can extend an existing registration by that amount." )
            syncOut( println( "Try again. Or just press [enter] to abort." ) )
            doQueryDurationInSecondsPaymentInWei
          }
          else {
            val paymentInWei = interactiveAssertAcceptablePayment( is, chainId, epp, name, seconds, unit, baseCurrencyCode, rmd ) // syncOut internal
  ...
  private def ensNameExtendTask( config : Configuration ) : Initialize[InputTask[Unit]] = {
    ...
    Def.inputTask {
      ...
      def doNameRenew( name : String, rmd : ensClient.RegistrarManagedDomain ) : Unit = {
        ...
        val svu = queryDurationInSeconds( is, """For how long would you like to extend the name (ex: "3 years")? """ ).getOrElse( aborted( "User failed to supply a desired time interval." ) )
        val seconds = svu.seconds
        val paymentInWei = interactiveAssertAcceptablePayment( is, chainId, epp, name, seconds, svu.unitProvided, baseCurrencyCode, rmd )
    ...
    val shouldUpdate = {
      oldValue match {
        ...
        case Some( differentAddress ) => {
          kludgeySleepForInteraction()
          val replace = queryYN( is, s"The alias '${alias}' currently points to address '${hexString(differentAddress)}' (for chain with ID ${chainId}). Replace? [y/n] " ) // internal syncOut
          if (! replace ) {
  ...
  private def ethAddressBalanceTask( config : Configuration ) : Initialize[InputTask[BigDecimal]] = {
    ...
    Def.inputTask {
      ...
      // uses syncOut internally where necessary
  ...
  private def ethKeystoreWalletV3CreateTask( config : Configuration ) : Initialize[Task[Unit]] = Def.taskDyn {
    ...
    interactiveOptionalCreateAliasTask( config, wallet.address ) // syncOut internal
  }
  ...
  private def ethKeystoreWalletV3FromPrivateKeyImportTask( config : Configuration ) : Initialize[Task[Unit]] = Def.taskDyn {
    ...
    val privateKeyStr = {
      val raw = assertReadLine( is, "Please enter the private key you would like to import (as 32 hex bytes): ", mask = true )
      if ( raw.startsWith( "0x" ) ) raw.substring(2) else raw
    }
    val privateKey = EthPrivateKey( privateKeyStr )
    val address = privateKey.address

    val confirm = assertReadLine( is, s"The imported private key corresponds to address '${hexString( address )}'. Is this correct? [y/n] ", mask = false ).equalsIgnoreCase("y")
    ...
    if (! confirm ) {
      ...
      val passphrase = syncOut {
        println( s"Generating V3 wallet, algorithm=pbkdf2, c=${c}, dklen=${dklen}" ) // use println rather than log info to be sure this prints before the credential query
  ...
  private def ethKeystoreWalletV3PrintTask( config : Configuration ) : Initialize[InputTask[Unit]] = Def.inputTask {
    val log = streams.value.log
    val keystoreDirs = activeShoeboxKeystoreV3()
  ...
  private def ethKeystoreWalletV3ValidateTask( config : Configuration ) : Initialize[InputTask[Unit]] = {
    ...
    Def.inputTask {
      ...
      if ( wallets.isEmpty ) {
        ...
        val credential = readCredential( is, inputAddress, acceptHexPrivateKey = false ) // syncOut internal
  ...
  def ethShoeboxSqlDumpCreateTask : Initialize[Task[Unit]] = Def.task {
    val log = streams.value.log
  ...
  def ethShoeboxSqlDumpRestoreTask : Initialize[Task[Unit]] = Def.task {
    val is  = interactionService.value
    val log = streams.value.log

    def interactiveListAndSelectDump( dumps : immutable.SortedSet[shoebox.Database.Dump] ) : Option[shoebox.Database.Dump] = syncOut {
  ...
  private def ethShoeboxBackupTask : Initialize[Task[Unit]] = Def.task {
    ...
    def queryCreateBackupDir( bd : File, alreadyDefault : Boolean ) : Option[File] = syncOut {
    ...
    def promptForNewBackupDir( promptToSave : Boolean = true ) : File = {
      ...
      val existingDir = {
        if ( !putativeDir.exists() ) {
          val toCreate = queryCreateBackupDir( putativeDir, alreadyDefault = false ) // syncOut internal
          toCreate match {
            case Some( f ) => f
  ...
  private def ethShoeboxDirectorySwitchTask : Initialize[InputTask[Unit]] = Def.inputTask {
    val log = streams.value.log
    val is  = interactionService.value
    ...
    if (! shoeboxFile.exists()) {
      val check = queryYN( is, s"Directory '${shoeboxFile}' does not exist. Create? [y/n] " )
      if (!check) aborted( s"User chose not to create putative shoebox directory '${shoeboxFile}'." )
    }

    val oopsie = shoebox.Shoebox.whyBadShoeboxDir( shoeboxFile ) // creates the directory!
    oopsie match {
  ...
  private def ethShoeboxRestoreTask : Initialize[Task[Unit]] = Def.taskDyn {
        ...
        if ( mostRecent.dbDumpSucceeded ) {
          ...
          if ( backupFilesAllGood.nonEmpty ) {
            ...
            val mbNum = syncOut {
              ...
              queryPositiveIntOrNone( is, "Enter a number, or hit return to abort: ", 1, 3 )
            }
            mbNum.flatMap { num =>
              num match {
    ...
    def promptViaBackupOrDir() : Option[File] = {
      ...
      if ( rawPath.isEmpty ) {
        ...
        if (! putative.exists) {
          syncOut( println( s"Selected file '${putative}' does not exist. Try again." ) )
      ...
      def doSpawn( deploymentAlias : String, codeHex : String, inputs : immutable.Seq[String], valueInWei : Unsigned256, abi : Abi ) : ( String, Either[EthHash,Client.TransactionReceipt] ) = {
        ...
        val f_out = {
          for {
            ...
            receipt <- Invoker.futureTransactionReceipt( txnHash ).map( ctr => prettyPrintEval( log, Some(abi), txnHash, invokerContext.pollTimeout, ctr ) )
          } yield {
        ...
        if ( !ephemeralDeployment && interactive ) {
          out match {
            case Right( ctr ) => {
              ...
              interactiveSetAliasForAddress( chainId )( s, log, is, s"the newly deployed '${deploymentAlias}' contract at '${hexString(address)}'", address ) // syncOut internal
            }
            case Left( _ ) => ()
          }
        }
  ...
  private def doReadMarkup( log : sbt.Logger, is : sbt.InteractionService, overObject : String, limitOrPrice : String ) : Float = {
    val markupStr = assertReadLine( is, s"Enter a markup over ${overObject} (as a fraction, e.g. 0.2, or a percentage, e.g. 20%): ", mask = false ) // syncOut internal
  ...
  private def ethTransactionGasLimitOverrideSetTask( config : Configuration ) : Initialize[InputTask[Unit]] = Def.inputTask {
    ...
    val ovr = {
      mbAmount match {
        ...
        case None           => {
          val mbFixed = assertReadOptionalBigInt( is, "Enter a fixed gas limit override, or hit [Enter] to specify a dynamic markup with optional cap and floor: ", mask = false ) // syncOut internal
          mbFixed match {
            case Some( fixed ) => Invoker.Override( fixed )
            case None          => {
              val markup = doReadMarkup(log, is, "estimated gas costs", "limit") // syncOut internal
              val cap = assertReadOptionalBigInt( is, "Enter a cap for the acceptable gas limit (or [Enter] for no cap): ", mask = false ) // syncOut internal
              val floor = assertReadOptionalBigInt( is, "Enter a floor for the acceptable gas limit (or [Enter] for no floor): ", mask = false ) // syncOut internal
    ...
    val ovr = {
      mbAmount match {
        ...
        case None           => {
          val mbFixed = {
            // syncOut internal
            assertReadOptionalAmountInWei( is, "Enter a fixed gas price override as amount and unit (e.g. '5 gwei'), or hit [Enter] to specify a dynamic markup with optional cap and floor: ", mask = false )
          }
          mbFixed match {
            case Some( fixed ) => Invoker.Override( fixed )
            case None          => {
              val markup = doReadMarkup(log, is, "default gas price", "price") // syncOut internal
              val cap = assertReadOptionalAmountInWei( is, "Enter a cap (e.g. '10 gwei') for the acceptable gas price (or [Enter] for no cap): ", mask = false ) // syncOut internal
              val floor = assertReadOptionalAmountInWei( is, "Enter a floor (e.g. '1 gwei') for the acceptable gas price (or [Enter] for no floor): ", mask = false ) // syncOut internal
              Invoker.Markup( fraction = markup, cap = cap, floor = floor )
            }
          }
        }
  ...
  private def ethTransactionSignTask( config : Configuration ) : Initialize[InputTask[EthTransaction.Signed]] = Def.inputTask {
    ...
    val signer : EthAddress = {
      ...
      if ( defaultToMaybeUse.nonEmpty ) {
        ...
        val raw = assertReadLine( is, "Signer (as hex address, ens-name, or alias): ", mask = false) // syncOut internal
    ...
    val chainId = {
      ...
      if ( useSessionChainId ) {
        ...
        val raw = assertReadLine( is, "Enter the Chain ID to sign with (or return for none, no replay protection): ", mask = false) // syncOut internal
        if (raw.nonEmpty) {
          val asNum = raw.toInt
    ...
    def querySignedTransactionFile : Option[File] = syncOut {
  ...
  private def querySaveAndPrintUnsignedTransaction( is : sbt.InteractionService, log : sbt.Logger, chainId : Int, nonceOverridden : Boolean, autoNonceAddress : EthAddress, unsigned : EthTransaction.Unsigned ) : Unit = {
    ...
    def queryUnsignedTransactionFileToCreate( is : sbt.InteractionService ) : Option[File] = syncOut {
      queryOptionalGoodFile (
        is = is,
  ...
  private def ethTransactionUnsignedInvokeTask( config : Configuration, functionFilter : jsonrpc.Abi.Function => Boolean ) : Initialize[InputTask[EthTransaction.Unsigned]] = {
    ...
    Def.inputTask {
      ...
      querySaveAndPrintUnsignedTransaction( is, log, chainId, nonceOverride.nonEmpty, caller, unsigned ) // syncOut internal
  ...
  private def ethTransactionUnsignedRawTask( config : Configuration ) : Initialize[InputTask[EthTransaction.Unsigned]] = {
    ...
    Def.inputTask {
      ...
      querySaveAndPrintUnsignedTransaction( is, log, chainId, nonceOverride.nonEmpty, from, unsigned ) // syncOut internal
  ...
  private def ethTransactionEtherSendTask( config : Configuration ) : Initialize[InputTask[Client.TransactionReceipt]] = {
    ...
    Def.inputTask {
      ...
      val f_out = Invoker.transaction.sendWei( lazySigner, to, Unsigned256( amount ), nonceOverride ) flatMap { txnHash =>
        ...
        log.info( s"Waiting for the transaction to be mined (will wait up to ${invokerContext.pollTimeout})." )
        Invoker.futureTransactionReceipt( txnHash ).map( ctr => prettyPrintEval( log, None, txnHash, invokerContext.pollTimeout, ctr ) )
      }
      val out = Await.result( f_out, Duration.Inf ) // we use Duration.Inf because the Future will complete with failure on a timeout
      log.info("Ether sent.")
      out
    ...
    Def.inputTask {
      ...
      val f_out = Erc20.lookupDecimals( tokenContractAddress ).recover( noDecimalsChecker( log, is, chainId, tokenContractAddress, rawNumStr, "approve" ) ).flatMap { decimalsUnsigned8 =>
        ...
        Erc20.doApprove( tokenContractAddress, lazySigner, approveAddress, numAtoms, nonceOverride ) flatMap { txnHash =>
          ...
          log.info( s"Waiting for the transaction to be mined (will wait up to ${invokerContext.pollTimeout})." )
          Invoker.futureTransactionReceipt( txnHash ).map( ctr => prettyPrintEval( log, Some(Erc20.Abi), txnHash, invokerContext.pollTimeout, ctr ) )
        }
      }
  ...
  private def xensClientTask( config : Configuration ) : Initialize[Task[ens.Client]] = Def.task {
    ...
    val ossF = stubOnTransactionSubmitted( log, icontext )

    // for now, we'll hard-code the stub context defaults 
    // we can make this stuff configurable someday if it seems useful
    implicit val scontext = stub.Context( icontext, stub.Context.Default.EventConfirmations, MainScheduler, ossF  )

    new ens.Client( nameServiceAddress )
  }

  private def stubOnTransactionSubmitted( log : sbt.Logger, icontext : Invoker.Context ) : Function1[Try[EthHash],Unit] = { tTxnHash =>
    tTxnHash match {
      case Success(txnHash) => {
        log.info( s"A transaction with hash '${hexString(txnHash)}' has been submitted." )
        log.info( s"Waiting up to ${icontext.pollTimeout} for the transaction to be mined." )
      }
      case Failure( e ) => {
        val msg = "Attempt to create and submit a transaction has failed."
        WARNING.log( msg, e )
        log.warn( s"${msg} ${e}" )
      }
    }
  ...
  private val xethKeystoreWalletV3CreatePbkdf2Task : Initialize[Task[wallet.V3]] = Def.task {
    ...
    log.info( s"Generating V3 wallet, algorithm=pbkdf2, c=${c}, dklen=${dklen}" )
  ...
  private def xethStubEnvironmentTask( config : Configuration ) : Initialize[Task[Tuple2[stub.Context,stub.Sender.Signing]]] = Def.task {
    ...
    val scontext = stub.Context( preapprovingInvokerContext, stub.Context.Default.EventConfirmations, MainScheduler, stubOnTransactionSubmitted( log, icontext ) )
  ...
  private def doShoeboxResetInitVerifyRecover( log : sbt.Logger, is : sbt.InteractionService, overrideShoeboxDirPath : Option[String] ) : Unit = {
    ...
    if ( schemaOkay.isFailed ) {
      ...
      if ( activeShoebox.database.schemaVersionInconsistentUnchecked.assert ) {
        ...
        log.error( "The sbt-ethereum shoebox sql database schema is in an inconsistent state (probably because an upgrade failed)." )
        val mbLastDump = activeShoebox.database.latestDumpIfAny.assert
        mbLastDump match {
          case Some( dump ) => {
            val dumpTime = formatInstant( dump.timestamp )
            kludgeySleepForInteraction()
            val recover = queryYN( is, s"The most recent sbt-ethereum shoebox sql database dump available was taken at ${dumpTime}. Attempt recovery? [y/n] " )
            if ( recover ) {
  ...
  private def readV3Wallet( is : sbt.InteractionService ) : wallet.V3 = {
    val jsonStr = assertReadLine( is, "V3 Wallet JSON: ", mask = false ) // internal syncOut
