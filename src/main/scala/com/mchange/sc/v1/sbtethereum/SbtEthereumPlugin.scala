...
object SbtEthereumPlugin extends AutoPlugin {
  ...
  object autoImport {
    ...
    val ethRepositoryDatabaseDump    = taskKey[Unit] ("Dumps the sbt-ethereum repository database as an SQL text file, stored inside the sbt-ethereum repository directory.")
    val ethRepositoryDatabaseRestore = taskKey[Unit] ("Restores the sbt-ethereum repository database from a previously generated dump.")
  ...
  lazy val ethDefaults : Seq[sbt.Def.Setting[_]] = Seq(
    ...
    ethRepositoryDatabaseDump := { ethRepositoryDatabaseDumpTask.value },

    ethRepositoryDatabaseRestore := { ethRepositoryDatabaseRestoreTask.value },
  ...
  def ethRepositoryDatabaseDumpTask : Initialize[Task[Unit]] = Def.task {
    val log = streams.value.log
  ...
  def ethRepositoryDatabaseRestoreTask : Initialize[Task[Unit]] = Def.task {
    val is  = interactionService.value
    val log = streams.value.log
